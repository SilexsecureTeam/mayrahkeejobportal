              ...
              {field_sections.primary?.map((currentKey) => {
                ...
                return (
                  <div className="flex flex-col gap-1">
                    <label>
                      {currentKey.name}
                      {currentKey?.field_name !== "middle_name" && (
          ...
          <div className="flex flex-col gap-5 border-b pb-4">
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-5">
              {field_sections.professional.map((currentKey, index) => {
                const detail = profileDetails[currentKey.field_name];
                const inputType =
                  currentKey.field_name === "member_since" ? "date" : "text";
                return (
                  <div className="flex flex-col gap-1" key={index}>
                    <label>
                      {currentKey.name}
                      {currentKey?.field_name !== "business_registration_no" &&
                        currentKey?.field_name !== "business_email" && (
                          <span className="text-red-500 ml-1">*</span>
                        )}
                    </label>
                    {currentKey.type !== "select" ? (
