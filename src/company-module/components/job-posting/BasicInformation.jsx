...
function BasicInformation({ setCurrentStep, data, jobUtils }) {
  ...
  const [subSectorList, setSubSectorList] = useState(jobUtils?.details?.sector ? jobSectors?.find(one=> one?.name === jobUtils?.details?.sector)?.subsections : selectedSector?.subsections);
  const [selectedSubSector, setSelectedSubSector] = useState(jobUtils?.details?.subsector ? subSectorList?.find(one=>one.name === jobUtils?.details?.subsector): subSectorList[0]);
  ...
  useEffect(() => {
    ...
    initData();
    setSubSectorList(selectedSector[0]);
