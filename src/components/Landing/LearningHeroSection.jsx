    ...
    return (
        <section className="bg-gray-100">
            ...
            <div className="flex space-x-4 mt-6 border-b-[1px] border-b-gray-400 overflow-x-auto">
                {
                    list?.map((course) => (
                        <button key={course?.id} onClick={()=>setSelected(course?.id)} className={`capitalize text-sm px-6 py-2 ${selected === course?.id ? "text-green-600 border-b-[2px] border-b-green-600" : "text-gray-700"} font-bold rounded`}>
                            {course?.title}
                        </button>
                    ))
                }
